...
% Finding the shortest path from nodes to base station
for i=1:1:n+1
min_hd=dist(i);
  f=1;
  l=1;
...
Y2(cluster2)=S2(i).yd;
            cluster2=cluster2+1;
% Calculation of energy dissipated in all the nodes involved in the multihop transmission
distance;
if ((distance>do))
                d=1;
                S2(shortpath(i,d)).E=S2(shortpath(i,d)).E- 1.5*( (ETX)+((EDA)*(4000))  + Efs*4000*( distance * distance ));
for d=2:1:5
if(shortpath(i,(d))==(n+1)) break; end
...
cluster3=1;
for i=1:1:n
if((S3(i).E>=(El3(r+1)/allive3)))
% Checking if the node’s energy is greater than the average node energy in the network
if((temp_rand(i,(r+1))<= (p/(1-p*mod(r,round(1/p))))))
...
C3(cluster3).id=i;
C3(cluster3).members=0;% number of nodes in the cluster
X3(cluster3)=S3(i).xd;
Y3(cluster3)=S3(i).yd;
cluster3=cluster3+1;
...
% Distribution of cluster nodes equally
if( C3(c).members>(n/ countCHs3)) continue; end
...
legend('LEACH','Multihop LEACH','LEACH-B');
